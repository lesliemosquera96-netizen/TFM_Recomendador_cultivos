...
# Estados pequeños del noreste que se etiquetan FUERA del mapa con flecha.
# Se colocan en una COLUMNA ordenada a la derecha (misma lon, latitudes
# equiespaciadas) para que NO se solapen entre sí. El texto se alinea a la
# derecha del punto de anclaje (textposition="middle right").
externos = {
    "VT": (47.0, -67.5), "NH": (45.6, -67.5), "ME": (44.2, -67.5),
    "MA": (42.8, -67.5), "RI": (41.4, -67.5), "CT": (40.0, -67.5),
    "NJ": (38.6, -67.5), "DE": (37.2, -67.5), "MD": (35.8, -67.5),
}
...
fig = go.Figure(data=go.Choropleth(
    ...
    colorscale="Greens",
    marker_line_color="rgb(110,110,110)",   # borde gris: distingue estados claros (Alaska, Hawái)
...
for _, row in df.iterrows():
    ...
    fig.add_trace(go.Scattergeo(
        ...
        mode="text",
        textfont=dict(size=15, color=color, family="Arial"),
...
for ab, (tlat, tlon) in externos.items():
    ...
    fig.add_trace(go.Scattergeo(
        ...
        mode="lines",
        line=dict(width=1.1, color="gray"),
        showlegend=False, hoverinfo="skip",
    ))
    # Cuadro de texto (alineado a la derecha del anclaje para formar columna)
    fig.add_trace(go.Scattergeo(
        lon=[tlon], lat=[tlat],
        text=f"<b>{row['estado']}</b>  {row['valor_txt']}",
        mode="text",
        textposition="middle right",
        textfont=dict(size=14, color="#0b3d0b", family="Arial"),
        showlegend=False, hoverinfo="skip",
    ))
...
fig.update_layout(
    ...
    # Lienzo grande + relación de aspecto apaisada (ideal para figura horizontal)
    width=2000, height=1250,
    margin=dict(l=150, r=20, t=110, b=10),
)
# Barra de color al LADO IZQUIERDO (el derecho queda libre para las etiquetas)
fig.data[0].colorbar.x = 0.0
fig.data[0].colorbar.xanchor = "right"
fig.data[0].colorbar.len = 0.7
fig.data[0].colorbar.title.font.size = 18
fig.data[0].colorbar.tickfont.size = 16

# --- EXPORTACIÓN PARA LA MEMORIA DEL TFM ---
# Vectorial (recomendado para insertar en el documento: nítido a cualquier zoom)
fig.write_image("mapa_produccion_agricola_nt.pdf")            # PDF vectorial
fig.write_image("mapa_produccion_agricola_nt.svg")            # SVG vectorial
# Mapa de bits en alta resolución (por si tu editor no admite vectoriales)
fig.write_image("mapa_produccion_agricola_nt.png", scale=3)   # ~4200x2700 px
# Versión interactiva (para verlo en el navegador)
fig.write_html("mapa_produccion_agricola_nt.html")
